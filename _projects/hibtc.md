...
\begin{enumerate}
    \item The endeffector position $p_e \in \mathbb{R}^3 \implies m_1=3$
    \item The endeffector orientation in euler angles $\phi_e \in \mathbb{R}^3 \implies m_2=3$
    \item The first joint angle $q_1 \in \mathbb{R} \implies m_3=1$
\end{enumerate}
...
\begin{enumerate}
    \item Comparison of two target trajectories, where one, as described in equation \ref{eq:des_traj}, has no conflicting tasks and the other does. The conflicting trajectory is realized by changing the last task to $x_{7} = x_{init, 7} + 2\pi\, \sin(\omega t)$, which results in a oscillation of the first joint with two full rotations.
    \item Investigation of the influence of the $\gamma_i(q, \dot{q})$-term by running one simulation including the  $\gamma_i$-term and one without it.
    \item External forces were applied at different task levels to examine their effects on the controller in both cases, as described along with equation \ref{eq:F_ext}. In each interval the corresponding force vector part $\mathbf{F}_{\dot{x}_j}^{\text{ext}} = \begin{bmatrix}300& 300 & 0 &60 & 0 & 0 & 100\end{bmatrix}^T$ was applied on a different task level.
\end{enumerate}
\begin{figure} [h!]
    \centering
